...
function chngUniq(num)
{
	if(num==1)
	{
		$('#typ_uniqid').text("Aadhar Card Number: ")
	}
	else if(num==2)
	{
		$('#typ_uniqid').text("Hospital Registration Number: ")
	}
	else
	{
		$('#typ_uniqid').text("Pharamacy Registration Number: ")
	}
}
...
function register()
	{
		...
		if(validatePassword(pwd) && validateName(nm))
		{
			$.ajax({
				...
				method:'POST',
				data:{name:$('#r_nm').val(),username:$('#r_name').val(),password:$('#r_pwd').val(),uniqid:$('#uniqid').val(),type:$("input[name='typeR']:checked").val()},
